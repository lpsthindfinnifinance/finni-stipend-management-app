...
- **Two-Tier Allocation System**: Supports two allocation types: (1) **Practice-to-Practice** allocations where PSMs and Lead PSMs can transfer stipends between practices within their own portfolio by selecting specific donor practices with amounts and specific recipient practices with amounts. The UI provides two separate tables for donor and recipient selection, with amount inputs for each practice. Total donor amount must equal total recipient amount. The system creates `allocation_out` ledger entries for each donor practice (negative amounts) and `allocation_in` ledger entries for each recipient practice (positive amounts). Donor practices are automatically excluded from the recipient list to prevent self-allocation. (2) **Inter-Portfolio** allocations where PSMs can transfer stipends to another portfolio's suspense account for later distribution. Lead PSMs can then allocate funds from their portfolio's suspense account to individual practices within their portfolio using an inline distribution form on the allocation detail page. The distribution form includes practice selection, amount inputs per practice, real-time balance validation (showing available, distributed, and remaining amounts), and comprehensive validation preventing null/negative/zero amounts. Upon distribution, the system creates `suspense_out` ledger entry (negative amount from portfolio suspense) and `allocation_in` ledger entries (positive amounts to recipient practices), then marks the allocation as completed. All allocations automatically create proper ledger entries with transaction types (allocation_out, allocation_in, suspense_in, suspense_out) and enforce balance validation in both UI and backend. Finance and Admin roles can allocate from any practice regardless of portfolio. The system pre-fetches portfolio data for optimal user experience when switching between allocation types. **Allocation detail pages are separated by direction**: `/allocations/in/:id` shows incoming allocations (funds received by user's current portfolio) with distribution functionality, while `/allocations/out/:id` shows outgoing allocations (funds sent from user's current portfolio) in view-only mode. The allocations table filters inter-portfolio allocations based on the user's currently selected portfolio tag, ensuring PSMs only see relevant incoming and outgoing allocations for their active portfolio context.
