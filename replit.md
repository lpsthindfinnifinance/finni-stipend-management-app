...
- **Pay Period Management**: Manages 14-day pay periods (26 periods per year), triggering BigQuery data imports, remeasurement calculations, and automatic ledger entries. All stipend requests are tied to specific pay periods. **Pay Period Calendar**: PP1 starts December 23, 2024 and ends January 5, 2025, with each subsequent period following 14-day intervals. **CSV Data Storage**: When Finance users import BigQuery CSV data for a pay period, the raw CSV is stored in the database and can be downloaded later via the "Download CSV" button in the All Pay Periods table on the Finance Ops page. The download endpoint is secured with Finance/Admin role authorization.
