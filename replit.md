...
- **Portfolio-Based Access Control**: PSM users can only view practice details, balances, ledgers, pending requests, and stipend request details for practices within their assigned portfolio. Lead PSM, Finance, and Admin users can view all practices and requests across all portfolios. When PSM users attempt to access practices or requests outside their portfolio, they receive a 403 error with an "Access Denied" toast notification and are redirected to /dashboard (for practices) or /requests (for stipend requests) after 1.5 seconds. The backend enforces this access control on all relevant API endpoints, and the frontend gracefully handles 403 responses with user-friendly error messages.
