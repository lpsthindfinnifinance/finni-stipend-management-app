  ...
  async getDashboardSummary(userId: string, role: string, portfolioId?: string): Promise<any> {
    ...
    const [
      ...
      db.select({
          ...
          totalGuaranteed: sql<number>`COALESCE(ABS(SUM(CASE WHEN ${stipendRequests.stipendType} = 'guaranteed_earnings' THEN CAST(${practiceLedger.amount} AS DECIMAL) ELSE 0 END)), 0)`
        })
        .from(practiceLedger)
        .leftJoin(stipendRequests, eq(practiceLedger.relatedRequestId, stipendRequests.id))
        .where(and(
  ...
  async getPortfolioSummaries(): Promise<any[]> {
    ...
    const summaries = portfolioList.map((portfolio) => {
      ...
      for (const practice of portfolioPractices) {
        ...
        // Get paid (including opening_balance_stipend_paid) and committed from ledger
        const ledger = ledgerByPractice[practice.id] || {};
        stipendPaid += (ledger['paid'] || 0) + (ledger['opening_balance_stipend_paid'] || 0);
