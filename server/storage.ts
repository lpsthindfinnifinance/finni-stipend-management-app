  ...
  async getDashboardSummary(userId: string, role: string, portfolioId?: string): Promise<any> {
    ...
    const [
      ...
      db.select({
          ...
          totalGuaranteed: sql<number>`COALESCE(ABS(SUM(CASE WHEN ${stipendRequests.requestType} = 'guaranteed_earnings' THEN CAST(${practiceLedger.amount} AS DECIMAL) ELSE 0 END)), 0)`
        })
        .from(practiceLedger)
